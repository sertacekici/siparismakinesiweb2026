...
const fields = [
  { name: "name", label: "Ortak Adı", type: "text" as const, required: true },
  {
    name: "description",
    label: "Açıklama",
  ...
  { name: "websiteUrl", label: "Web Sitesi", type: "url" as const },
  {
    name: "isActive",
    label: "Aktif",
...
export default function OrtaklarPage() {
  const { data, loading, add, update, remove } = useFirestore("partners");
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<Record<string, unknown> | null>(null);
  const [saving, setSaving] = useState(false);

  const columns = [
    { key: "name", label: "Ortak Adı" },
    {
      key: "websiteUrl",
      label: "Web Sitesi",
      render: (val: unknown) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          <Plus className="h-4 w-4" /> Yeni Ortak
        </button>
      </div>
      ...
      <DataTable
        ...
        onDelete={(row) => {
          if (confirm("Bu ortağı silmek istediğinize emin misiniz?")) {
      ...
      {modalOpen && (
        <CrudModal
          title={editing ? "Ortağı Düzenle" : "Yeni Ortak"}
          fields={fields}
